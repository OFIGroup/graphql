        ...
        describe("where", () => {
            describe("primitives", () => {
                ...
                test("_ANY", () => {
                    ...
                    expect(trimmer(result[0])).toEqual(
                        trimmer(`
                        ((this.ids IS NOT NULL AND this.ids = []) OR (this.ids IS NOT NULL AND ANY(x IN $this_auth_where0_ids_ANY WHERE x IN this.ids)) OR this.ids IS NULL)
                  `)
                    );
